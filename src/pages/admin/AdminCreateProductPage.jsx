...
const AdminCreateProductPage = () => {
  ...
  const { data: categoryData } = useQuery({
    queryKey: ['categories'],
    queryFn: () => categoryService.getCategoriesByPage(1, 100)
  });
  ...
  const validateForm = () => {
    ...
    if (!product.price) newErrors.price = 'O preço é obrigatório';
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-3">
                ...
                <input type="text" className={`form-control ${errors.title ? 'is-invalid' : ''}`}
                  name="title" value={product.title} onChange={handleChange} autoFocus />
                {errors.title && <div className="invalid-feedback">{errors.title}</div>}
              </div>
              <div className="mb-3">
                <label className="form-label">Descrição</label>
                <textarea className={`form-control ${errors.description ? 'is-invalid' : ''}`}
                  name="description" rows="3" value={product.description} onChange={handleChange}></textarea>
              ...
              <div className="mb-3">
                ...
                <button type="button" className="btn btn-outline-secondary btn-sm"
                  onClick={() => fileRef.current?.click()}>Selecionar arquivo</button>
              ...
              {(product.image_preview || product.image_url) && (
                <div className="mb-3 text-start">
                  <img src={product.image_preview || product.image_url} alt="Pré-visualização"
                    className="img-thumbnail" style={{ maxHeight: 200 }} />
                </div>
              )}
